    ...
    for batch_idx, (input_texts, target_sequences) in enumerate(train_dataloader):
        ...
        async with lock:
            ...
            with torch.no_grad():
                embeddings_list = [
                    encode_long_text(
                        text, model.embedding_model, train_dataloader.dataset.vocab.tokenizer, device
                    ) for text in input_texts
                ]
                input_embeddings = torch.cat(embeddings_list, dim=0)

            target_sequences = target_sequences.to(device)

            try:
                with autocast(device_type=device.type):
                    loss = await model.learn_one_step(input_embeddings, target_sequences, stop_event=training_state.stop_training_flag)
        ...
        for batch_idx, (input_texts, target_sequences) in enumerate(val_dataloader):
            ...
            embeddings_list = [
                encode_long_text(
                    ...
                    val_dataloader.dataset.vocab.tokenizer,
                    device
                ) for text in input_texts
            ]
            input_embeddings = torch.cat(embeddings_list, dim=0)
            ...
            with autocast(device_type=device.type):
                predicted_embedding = await model._get_predicted_embedding(input_embeddings)
